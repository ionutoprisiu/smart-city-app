...
from app.integrations.ai_client import AiServiceError, support_match as ai_support_match
...
from app.schemas.activities import ChatMessageCreateRequest, ChatMessageResponse, ChatPostResponse
...
def _maybe_autoreply_event(db: Session, event: ActivityEvent, message: ActivityChatMessage) -> ActivityChatMessage | None:
    return _maybe_autoreply(db, message, pairs=_event_qa_pairs(db, event.id), event_id=event.id)


def _maybe_autoreply_club(db: Session, club: Club, message: ActivityChatMessage) -> ActivityChatMessage | None:
    return _maybe_autoreply(db, message, pairs=_club_qa_pairs(db, club.id), club_id=club.id)


def _event_qa_pairs(db: Session, event_id: int) -> list[dict]:
    return _qa_pairs(db, event_id=event_id)


def _club_qa_pairs(db: Session, club_id: int) -> list[dict]:
    return _qa_pairs(db, club_id=club_id)


def _find_similar(message: str, pairs: list[dict]) -> dict | None:
    if not pairs:
        return None
    candidates = [{"questionId": p["questionId"], "question": p["questionBody"], "answer": p["answerBody"]} for p in pairs]
    try:
        data = ai_support_match(message=message, candidates=candidates)
    except AiServiceError:
        return None
    matched_id = data.get("matchedQuestionId")
    if matched_id is None:
        return None
    for p in pairs:
        if p["questionId"] == int(matched_id):
            return p
    return None


def _ensure_event(db: Session, event_id: int) -> ActivityEvent:
    event = db.execute(select(ActivityEvent).where(ActivityEvent.id == event_id, ActivityEvent.status != "DELETED")).scalar_one_or_none()
    if event is None:
