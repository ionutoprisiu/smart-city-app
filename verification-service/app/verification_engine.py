...
def _decode_image(data: bytes) -> np.ndarray:
    # Prefer PIL so EXIF orientation from iPhone camera is normalized.
    try:
...
def _enhance_for_detection(image: np.ndarray) -> np.ndarray:
    # CLAHE helps with low-contrast/printed portrait photos on ID cards.
...
def _extract_face_embedding(image: np.ndarray, *, id_card_mode: bool = False, source_label: str = "image") -> np.ndarray:
    ...
    if id_card_mode:
        # Romanian ID cards usually have the portrait on the left side.
...
def _cosine_to_score(cosine_similarity: float) -> float:
    # InsightFace outputs embeddings where cosine similarity near 1 means better match.
...
def warm_up_models() -> None:
    # Trigger model initialization/download on service startup instead of first user request.
    _face_app()
