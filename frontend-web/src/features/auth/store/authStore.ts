...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  async login(request) {
    ...
    try {
      ...
      StorageService.saveUserEmail(response.email);
      StorageService.saveUserName(response.firstName, response.lastName);
      if (response.role) StorageService.saveUserRole(response.role);
      if (response.isVerified != null) StorageService.saveUserIsVerified(response.isVerified);
      StorageService.saveUserVerificationStatus(response.verificationStatus);
      if (response.accessToken) StorageService.saveUserToken(response.accessToken);
      set({
        currentUser: {
  ...
  async refreshVerificationStatus() {
    ...
    try {
      ...
      StorageService.saveUserRole(status.role);
    } catch {}
  },
